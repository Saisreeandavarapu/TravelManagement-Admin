...
import {
  FiGrid, FiPackage, FiCalendar, FiCreditCard,
  FiStar, FiLogOut, FiMenu, FiX, FiBell,
  FiChevronRight, FiUser, FiMapPin, FiBarChart2
} from 'react-icons/fi';

const menuItems = [
  { name: 'Dashboard', path: '/customer/dashboard', icon: FiGrid, color: '#6366f1' },
  { name: 'Browse Packages', path: '/customer/packages', icon: FiPackage, color: '#06b6d4' },
  { name: 'My Bookings', path: '/customer/bookings', icon: FiCalendar, color: '#8b5cf6' },
  { name: 'Payments', path: '/customer/payments', icon: FiCreditCard, color: '#10b981' },
  { name: 'Reviews', path: '/customer/reviews', icon: FiStar, color: '#f59e0b' },
];

const NOTIFICATIONS = [
  { id: 1, title: 'Booking Confirmed!', sub: 'Your Goa Beach Tour has been confirmed.', time: '10m', unread: true, icon: FiCalendar, color: '#10b981' },
  { id: 2, title: 'Payment Successful', sub: 'Received payment for Goa trip.', time: '1h', unread: false, icon: FiCreditCard, color: '#6366f1' },
];
...
const CustomerLayout = () => {
  ...
  const avatarLetter = user?.firstName?.[0] || user?.name?.[0] || 'C';
  const displayName = user
    ? (user.firstName && user.lastName ? `${user.firstName} ${user.lastName}` : (user.name || 'Customer'))
    : 'Customer';
  const displayRole = 'Traveller Account';
        ...
        <div className="flex items-center justify-between px-5 h-16 flex-shrink-0"
          ...
          <Link to="/customer/dashboard" className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #06b6d4, #6366f1)', boxShadow: '0 0 20px rgba(6,182,212,0.4)' }}>
              <FiPackage className="w-5 h-5 text-white" />
            </div>
            <div>
              <p className="font-display font-bold text-base text-white leading-tight tracking-wide">
                Travel<span style={{ color: '#06b6d4' }}>Companion</span>
              </p>
              <p className="text-[10px] text-slate-400 font-medium leading-none">Customer Portal</p>
            </div>
          </Link>
            ...
            <div className="relative">
              <button
                ...
                className={`relative p-2 rounded-xl transition-all ${
                  isNotifOpen ? 'bg-cyan-50 text-cyan-600' : 'text-slate-500 hover:bg-slate-50'
                }`}
              >
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm text-white flex-shrink-0"
                  style={{ background: 'gradient-brand', boxShadow: '0 2px 8px rgba(6,182,212,0.3)' }}>
                  {avatarLetter.toUpperCase()}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-bold text-slate-800 leading-tight truncate max-w-[100px]">{displayName}</p>
                  <p className="text-[10px] text-slate-400 font-medium leading-none">Traveller</p>
                </div>
              </button>

              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl shadow-xl overflow-hidden border border-slate-100 bg-white z-50">
                  <div className="px-4 py-4 text-white" style={{ background: 'linear-gradient(135deg, #06b6d4, #0891b2)' }}>
                    <p className="font-bold text-sm">{displayName}</p>
                    <p className="text-[10px] text-cyan-100">{user?.email}</p>
                  </div>
                  <div className="p-2">
                    <button
                      onClick={() => { setIsProfileOpen(false); navigate('/customer/dashboard'); }}
